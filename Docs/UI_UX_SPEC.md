...
4. **Risk rose** — badge/pin/heat ใช้โทน rose แยกจาก urgency (ส้ม) และ rain (sky)
...
- **Token:** `METRIC_STRIP` (divide-x + ring) + cell tint: urgency `slate-50/85`, risk `rose-50/75`, rain `sky-50/75`
...
### Detail Panel Wide Split (2026-07-08)

- **Desktop `ReportDetailPanel`:** `layout="split"` — 2 คอลัมน์ `52% / 48%`
  - **ซ้าย:** ดูบนแผนที่, AI reason (14px, leading-relaxed)
  - **ขวา:** รูป `aspect-[16/10] max-h-[168px]`
  - **เต็มความกว้างใต้ body:** `MetricsBento` variant `panel` (แนวนอน 3 ช่อง)
- **Footer desktop:** `StatusSegmented` เต็มความกว้าง + ปุ่มบันทึก (`StatusActionBar showStatus={true}`)
- **Mobile `DetailSheet`:** คง `layout="stack"` (บนลงล่าง) + status ใน footer
- **Workspace:** `xl:grid-cols-[minmax(0,1fr)_minmax(520px,600px)]`

### Metrics Color Polish v2 — Rose Risk (2026-07-09)

แยกสี 3 การ์ดใน detail panel ให้ชัด: **เร่งด่วน (ส้ม)** / **ความเสี่ยง (rose)** / **โอกาสฝนตก (sky)**

| การ์ด | Accent | พื้นการ์ด | ไอคอนหัวข้อ |
|--------|--------|-----------|-------------|
| เร่งด่วน | `#F97316` / `#EA580C` | `slate-50` | `slate-500` + title **16px** |
| ความเสี่ยง | Rose `#E11D48` / `#BE123C` | `rose-50/75` | `slate-500` |
| โอกาสฝนตก | Sky `#0284C7` | `sky-50/75` | `slate-500` |

- **Token กลาง:** `lib/risk-colors.ts` — `RISK_RING`, `RISK_BADGE`, `METRIC_CELL_RISK`, `riskMetricValueClass`, `riskBarFillClass`, `riskBarTrackClass`
- **Consumers:** thumb ring (`status-colors`), `RiskScore`, `FilterTabs` severe tab, ปฏิทิน severe heat, map pins (`LeafletMap` → `RISK_RING`)
- **ไม่แตะ:** priority chip, `QueueTimeline`, urgency tier/badge, landing/report orange ที่ไม่ใช่ risk level
...
| เริ่มอุดตัน | rose-50 | rose-700 | `#FB7185` ring / `#BE123C` text family |
| อุดตันหนัก | rose-50 | rose-800 | `#E11D48` ring / `#BE123C` text family |
...
แบนเนอร์บนสุดของหน้า Background rose-50 (ใช้สีเดียวกับ Risk Badge ระดับอุดตันหนัก เพื่อความสอดคล้อง) Text rose-800 หายไปเองใน 4 วินาที
